...
$sql = "SELECT nama, email FROM user";
$result = mysqli_query($conn, $sql);
?> -->

<!--Header-->
<!-- <header class="flex flex-wrap sm:justify-start sm:flex-nowrap w-full bg-primary text-sm py-4 dark:bg-neutral-800">
    <nav class="max-w-[115rem] items-center w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between" aria-label="Global">
        <a class="flex-none" href="#"><img class="w-72" src="./asset/logo.png" alt="" /></a>
        <div class="flex flex-row items-center gap-10 text-2xl text-white mt-5 sm:justify-end sm:mt-0 sm:ps-5">
            ...
            <?php if (isset($_SESSION['email'])) : ?>
                <?php
                // Ambil informasi pengguna yang masuk
                $sql = "SELECT nama, email FROM user WHERE email = '{$_SESSION['email']}'";
                $result = mysqli_query($conn, $sql);
                ...
                <div class="hs-dropdown relative inline-flex">
                    ...
                    <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-60 bg-white shadow-md rounded-lg p-2 mt-2 dark:bg-neutral-800 dark:border dark:border-neutral-700" aria-labelledby="hs-dropdown-with-header">
                        <div class="py-3 px-5 -m-2 bg-gray-100 rounded-t-lg dark:bg-neutral-700">
                            ...
                            <p class="text-sm font-medium text-gray-800 dark:text-neutral-300"><?php echo $user["email"]; ?></p>
                        </div>
            ...
            <?php else : ?>
                <button class="py-2 px-10 font-bold bg-secondary rounded-lg hover:opacity-75">
...
<header class="flex flex-wrap sm:justify-start sm:flex-nowrap w-full bg-primary text-sm py-4 dark:bg-neutral-800">
    <nav class="max-w-[115rem] items-center w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between" aria-label="Global">
        ...
        <div class="flex flex-row items-center gap-10 text-2xl text-white mt-5 sm:justify-end sm:mt-0 sm:ps-5">
            ...
            <a class="font-medium  hover:text-gray-400 dark:text-neutral-400 dark:hover:text-neutral-500" href="#">Car</a>
            <a class="font-medium  hover:text-gray-400 dark:text-neutral-400 dark:hover:text-neutral-500" href="#">Contact</a>
            <button class="py-2 px-10 font-bold bg-secondary rounded-lg hover:opacity-75"><?php echo $_SESSION['email']; ?>!</button>
        </div>
    </nav>
</header>
